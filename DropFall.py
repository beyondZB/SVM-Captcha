...
def get_nearby_pix_value(img, x, y, j):
    """获取临近5个点像素数据"""
    img_pix = img.load()
    width, height = img.size
    neighbour_offset = [(0,0), (-1,1), (0,1), (1,1), (1,0), (-1,0)]
    (x_offset, y_offset) = neighbour_offset[j]
    tx = x + x_offset
    ty = y + y_offset
    if(tx < 0 or tx >= width or ty < 0 or ty >= height):
        return 0
    return 0 if img_pix[tx, ty] == 0 else 1
...
def get_end_route(img, start_x, height):
    ...
    while cur_p[1] < (height - 1):
        step += 1
        intertial_pre_pix = end_route[max(step - 5, 0)]
        sum_n = 0
        max_w = 0
        next_x = cur_p[0]
        next_y = cur_p[1]
        ...
        if max_w == 1:
            ...
            if intertial_pre_pix[0] > cur_p[0]:
                # 向右
                next_x = cur_p[0] + 1
                next_y = cur_p[1] + 1
            if intertial_pre_pix[0] < cur_p[0] or sum_n == 0:
                next_x = cur_p[0]
                next_y = cur_p[1] + 1
        else:
